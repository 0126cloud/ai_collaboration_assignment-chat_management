...
- [ ] `broadcasts` migration 正常執行
- [ ] Seed data 載入 3 筆廣播
- [ ] `testDb.ts` 已更新 broadcasts schema
- [ ] `BroadcastService.computeStatus()` 邏輯正確
- [ ] GET 列表回傳每筆含正確 `status`
- [ ] GET status 篩選正常運作
- [ ] POST 發送廣播（201）
- [ ] POST validation 失敗（400）
- [ ] DELETE 下架成功（200，軟刪除）
- [ ] DELETE 不存在回 404 `BROADCAST_NOT_FOUND`
- [ ] general_manager 存取 broadcast 路由回 403
- [ ] `permissions.ts` 已新增 `broadcast:delete`
- [ ] 操作紀錄寫入 `SEND_BROADCAST`、`DELETE_BROADCAST`

### 前端

- [ ] `broadcastApi` 三個方法正常
- [ ] `BroadcastPage` 列表顯示正確
- [ ] 狀態 Tag 顏色對應正確
- [ ] 發送廣播表單 Zod 驗證正常
- [ ] 下架按鈕顯示邏輯正確（只在 scheduled / active 顯示）
- [ ] Router 新增廣播頁路由
- [ ] `general_manager` 看不到廣播選單

### rfc_01 更新

- [ ] [rfc_01 §5.5](rfc_01-auth-and-response.md) 補充 `broadcast:delete`
- [ ] [rfc_01 §5.9](rfc_01-auth-and-response.md) 補充 `DELETE /api/broadcasts/:id`

### 測試

- [ ] `broadcast.test.ts` integration tests 全部通過
- [ ] `BroadcastPage.test.tsx` component tests 全部通過
- [ ] `npm test` 全部通過
