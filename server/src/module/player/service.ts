...
export class PlayerService {
  ...
  async listNicknameReviews(query: TNicknameReviewQuery) {
  ...
  async approveNickname(username: string, operator: string) {
  ...
  async rejectNickname(username: string, operator: string) {
    const player = await this.db('players').where({ username }).whereNull('deleted_at').first();
